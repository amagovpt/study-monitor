...
@Component({
  ...
  templateUrl: './tag-statistics-websites-results.component.html',
  styleUrls: ['./tag-statistics-websites-results.component.css']
})
export class TagStatisticsWebsitesResultsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let counted_websites = {};
    for (let w in websites) {
      for (let p of websites[w]) {
        let tot = JSON.parse(atob(p.Tot));
        let perrors = tot.elems;

        for (let t in tot.results) {
          let v = tot.results[t];
          if (!this.results[t]) {
            this.results[t] = v; 
          }
    ...
    for (let h in this.comboio) {
      for (let t in this.comboio[h]) {
        ...
        let desc = t;
        let result = this.results[t];
        let test = this.tests[t]['test'];
        
        let n_elems = this.errors[test]['n_elems'];
        let n_pages = this.errors[test]['n_pages'];
        let n_websites = this.errors[test]['n_websites'];
        let lvl = _.toUpper(this.tests[t]['level']);

        let s = _.split(result, '@');

        let _class = parseInt(s[0], 0) === 10 ? 'scoreok' : parseInt(s[0], 0) < 6 ? 'scorerror' : 'scorewar';
        let prio = parseInt(s[0], 0) === 10 ? 3 : parseInt(s[0], 0) < 6 ? 1 : 2;
        let quartiles = this.calculateQuartiles(this.getErrorOcurrenceByWebsite(test));
  ...
  getErrorOcurrenceByWebsite(error: string): Array<number> {

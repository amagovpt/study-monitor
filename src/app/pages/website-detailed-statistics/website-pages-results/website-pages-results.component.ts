  ...
  ngOnInit(): void {
    ...
    const errors = _.map(this.errors, (v, k) => {
      return {
        ...
        'lvl': this.elemStats[k]['lev'],
        'quartiles': this.calculateQuartiles(this.getErrorOcurrenceByPage(k))
      };
    });

    this.errors = _.orderBy(errors, ['n_pages', 'n_elems'], ['desc', 'desc']);
  }
